...
export async function salvar(
  ...
  for (const f of FONTES) {
    ...
    sources[f.nome] = {
      ...
      rejeitar_divulgacao: triEstado(form, `divulgacao_${f.nome}`),
      reject_english: triEstado(form, `ingles_${f.nome}`),
      aceitar_sem_local: triEstado(form, `sem_local_${f.nome}`),
  ...
  const cfg: ConfigBot = sanear({
    ...
    rejeitar_divulgacao: marcado(form, 'rejeitar_divulgacao'),
    aceitar_sem_local: marcado(form, 'aceitar_sem_local'),
    // Um checkbox por UF: `getAll` devolve só as marcadas.
    ufs_atendidas: form.getAll('ufs_atendidas').map(String),
    sources,
  });

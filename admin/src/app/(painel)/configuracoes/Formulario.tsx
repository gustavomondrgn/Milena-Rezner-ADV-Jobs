...
import { CATEGORIAS, FONTES, UFS, type ConfigBot } from '@/lib/config-tipos';
    ...
    <form action={acao} className="flex flex-col gap-4 mt-6 pb-24">
      <Cartao titulo="Volume e horário">
        <p className="text-[13px] mb-5" style={{ color: 'var(--texto-suave)' }}>
          A demanda aprovada entra numa fila ordenada por nota. Com teto em{' '}
          <strong>0</strong> ela vai ao grupo assim que for aprovada; com teto
          maior que zero, o robô publica só as melhores e o resto concorre no
          dia seguinte.
        </p>

        <div className="grid gap-4 sm:grid-cols-3">
          <Numero
            nome="daily_limit" rotulo="Demandas por dia" padrao={inicial.daily_limit}
            min={0} max={200}
            ajuda="0 = sem teto (padrão). Acima de 0, o resto espera na fila."
          />
          <Numero
        ...
        <div className="mt-4 pt-4 border-t" style={{ borderColor: 'var(--borda)' }}>
          <Numero
            ...
            min={0} max={100}
            ajuda="Demanda com nota abaixo disso nem entra na fila. Deixe 0 para aceitar todas as aprovadas."
          />
        </div>
      </Cartao>
      ...
      <Cartao titulo="Regras gerais do filtro">
        ...
        <div className="flex flex-col gap-1">
          <Chave
            nome="rejeitar_divulgacao" padrao={inicial.rejeitar_divulgacao}
            rotulo="Recusar advogado se anunciando"
            ajuda="Currículo, tabela de honorários, 'atuo em todo o Brasil'. É o ruído dominante dos grupos — desligar isso enche o grupo de concorrentes."
          />
          <Chave
            nome="aceitar_sem_local" padrao={inicial.aceitar_sem_local}
            rotulo="Aceitar demanda presencial sem local declarado"
            ajuda="Em grupo regional quase ninguém escreve a cidade. Desligar aqui descarta toda demanda presencial que não disser onde é."
          />
          <Chave
            nome="reject_english" padrao={inicial.reject_english}
            rotulo="Recusar posts em inglês"
            ajuda="Vale para o idioma em que o post foi escrito."
          />
        </div>
      </Cartao>

      <Cartao titulo="Estados atendidos">
        <p className="text-[13px] mb-4" style={{ color: 'var(--texto-suave)' }}>
          Esta lista <strong>só é usada quando a demanda exige presença
          física</strong> — audiência, diligência, protocolo, assembleia.
          Trabalho que se faz de qualquer lugar (contrato, parecer, notificação,
          petição) passa independente do estado, porque o atendimento é 100%
          digital. Nenhum estado marcado = aceita todos.
        </p>
        <div className="flex flex-wrap gap-1.5">
          {UFS.map((uf) => (
            <CaixaUF key={uf} uf={uf} marcada={inicial.ufs_atendidas.includes(uf)} />
          ))}
        </div>
      </Cartao>
  ...
  return (
    <div className="rounded-xl border p-4" style={{ borderColor: 'var(--borda)' }}>
      ...
      <div className="grid gap-3 mt-4 sm:grid-cols-2 lg:grid-cols-4">
        <Tri nome={`divulgacao_${nome}`} rotulo="Recusar divulgação"
             padrao={regras.rejeitar_divulgacao} />
        <Tri nome={`sem_local_${nome}`} rotulo="Aceitar sem local"
             padrao={regras.aceitar_sem_local} />
...
/**
 * Uma UF como pílula clicável.
 *
 * Checkbox escondido com `peer` em vez de estado em React: o formulário inteiro
 * é não-controlado (Server Action lê o FormData), e 27 estados em JS só para
 * pintar um fundo seria trabalho para nada.
 */
function CaixaUF({ uf, marcada }: { uf: string; marcada: boolean }) {
  return (
    <label className="relative">
      <input
        type="checkbox" name="ufs_atendidas" value={uf} defaultChecked={marcada}
        className="peer sr-only"
      />
      <span
        className="inline-flex items-center justify-center min-w-[44px] rounded-lg border px-2.5 text-[13px] font-medium cursor-pointer select-none transition-colors
                   peer-checked:border-[var(--acento)] peer-checked:bg-[var(--acento)] peer-checked:text-white
                   peer-focus-visible:outline peer-focus-visible:outline-2 peer-focus-visible:outline-offset-2 peer-focus-visible:outline-[var(--acento)]"
        style={{
          borderColor: 'var(--borda)',
          color: 'var(--texto-suave)',
          // Alvo de toque por PONTEIRO, não por largura de tela: tablet de
          // 768px é tela grande e mesmo assim é dedo.
          minHeight: '38px',
        }}
      >
        {uf}

...
/**
 * A linha de contexto embaixo do título.
 *
 * Ordem escolhida por utilidade de relance: o tipo (cliente direto vale mais
 * que parceria), a área, onde é, e quanto paga. Campo vazio some — post de
 * Facebook tem quase sempre metade dos campos em branco, e escrever "não
 * informado" quatro vezes esconderia o que de fato foi informado.
 */
function linhaMeta(v: Vaga): string {
  const local = [v.comarca, v.uf].filter(Boolean).join('/');
  return [
    v.tipo_demanda ? rotuloTipoDemanda(v.tipo_demanda) : '',
    v.categoria ? rotuloCategoria(v.categoria) : '',
    local ? (v.exige_presenca ? `${local} · presencial` : local) : '',
    v.valor,
    v.autor,
  ].filter(Boolean).join(' · ') || '—';
}
...
const STATUS = [
  ...
  { valor: 'sent', rotulo: 'Enviadas' },
  { valor: 'queued', rotulo: 'Na fila' },
  { valor: 'skipped', rotulo: 'Descartadas' },
  { valor: 'divulgacao', rotulo: 'Divulgação' },
  { valor: 'local', rotulo: 'Fora da área' },
  { valor: 'categoria', rotulo: 'Área desligada' },
...
export default async function Vagas({ searchParams }: { searchParams: Promise<Busca> }) {
  ...
  return (
    <>
      <header>
        <h1 className="text-[22px] font-semibold tracking-tight">Demandas</h1>
        <p className="mt-0.5 text-[13px]" style={{ color: 'var(--texto-suave)' }}>
          Tudo que o robô leu — o que foi para o grupo e o que foi recusado, com o motivo.
        </p>
      </header>
      ...
      <div className="cartao mt-5 p-4 flex flex-col gap-3">
        <form action="/demandas" method="get" className="flex gap-2">
          ...
          <input
            ...
            placeholder="Buscar por texto, autor ou comarca…" aria-label="Buscar"
          />
        ...
        {/* O filtro por fonte só aparece se houver mais de uma. Com uma fonte
            só, ele seria um par de botões ("Todas" / "Facebook") que filtram
            exatamente o mesmo conjunto — ocupando espaço e sugerindo que existe
            uma escolha a fazer. */}
        {FONTES.length > 1 && (
      ...
      <div className="cartao mt-4 overflow-hidden">
        {r.linhas.length === 0 ? (
          <Vazio titulo="Nenhuma demanda encontrada"
                 texto="Nenhum registro bate com esses filtros. Tente limpar a busca ou escolher outra situação." />
        ) : (
          <>

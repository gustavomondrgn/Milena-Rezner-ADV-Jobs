...
class Store:
    ...
    def registrar_evento(self, *, uid: str, source: str, status: str, local_day: str,
                         title: str = "", autor: str = "", url: str = "",
                         category: str = "", categoria: str = "",
                         tipo_demanda: str = "", resumo_demanda: str = "",
                         uf: str = "", comarca: str = "",
                         exige_presenca: bool = False, tem_contato: bool = False,
                         valor: str = "", language: str = "", score: int = 0,
                         reason: str = "", published_at: str = "",
                         telegram_message_id: int | None = None) -> None:
        """Anota o destino de um post. Repetir o mesmo (uid, status) não duplica."""
        self._executar(
            """
            INSERT INTO job_events (
                uid, source, title, autor, url, status, category, categoria,
                tipo_demanda, resumo_demanda, uf, comarca, exige_presenca,
                tem_contato, valor, language, score, reason, published_at,
                local_day, telegram_message_id
            ...
            ON CONFLICT (uid, status) DO UPDATE SET
                ...
                created_at = now()
            """,
            (uid, source, title[:300], autor[:200], url[:600], status,
             category, categoria, tipo_demanda, resumo_demanda[:120],
             uf[:2], comarca[:120], bool(exige_presenca), bool(tem_contato),
             valor[:120], language, int(score), reason[:400],
             published_at[:60], local_day, telegram_message_id),
        )

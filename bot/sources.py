...
class AuthError(SourceError):
    """Sessão ou credencial inválida/expirada numa fonte que exige login.

    Vale para qualquer fonte logada. Hoje quem a levanta é o Facebook, quando a
    página que volta é a de login em vez do feed do grupo — o modo de falha mais
    traiçoeiro de um scraper, porque sem esta exceção ele devolveria zero post e
    pareceria apenas um dia quieto no grupo.
    """


# NOTA: aqui existia a fonte ONM (O Mercado de Trabalho), do projeto anterior.
# Foi removida por depender da conta paga de outro cliente e não ter nada a ver
# com demanda jurídica. Gupy, Indeed e LinkedIn ficaram: são portais genéricos,
# já testados em produção, desligados por padrão em `SOURCES`, e prontos caso um
# dia se queira também vaga de emprego formal de advogado.

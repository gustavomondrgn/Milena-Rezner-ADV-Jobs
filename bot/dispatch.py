...
class SendQueue:
    ...
    def proxima(self, *, agora: datetime, hoje: str, limite_diario: int,
        ...
        """A demanda que deve ir ao grupo agora, ou None se não é hora.

        `limite_diario <= 0` significa **sem teto**, que é o padrão deste
        projeto. Nesse modo caem as duas travas de volume — a cota e o
        espaçamento entre envios — e sobra apenas a janela de horário, para
        ninguém receber notificação às 4h da manhã.

        A fila continua servindo mesmo sem teto: ela ordena por nota, então numa
        rajada o cliente direto sai antes do "alguém indica um advogado?". E
        ligar o teto de volta é escrever um número no painel, não um deploy.
        """
        with self._lock:
            ...
            if limite_diario > 0:
                ...
                if self._ultimo_envio:
                    try:
                        desde = (agora - datetime.fromisoformat(
                            self._ultimo_envio)).total_seconds()
                    except ValueError:
                        desde = float("inf")

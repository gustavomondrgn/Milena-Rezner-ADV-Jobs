...
log = logging.getLogger("adv-jobs-bot.control")
    ...
    **Os escopos por chat também precisam ser apagados, um a um.** A versão
    ...
    _post(token, "setMyCommands", {
        "commands": [
            ...
            {"command": "suporte", "description": "Falar com o escritório"},
        ],
...
class CommandListener:
    ...
    def apresentacao(self) -> str:
        linhas = [
            "👋 Olá! Eu sou o <b>ADV Jobs</b>, o robô de demandas jurídicas da "
            "<b>Dra. Milena Rezner</b>.",
            "",
            "Eu acompanho grupos de advocacia o dia inteiro e levo ao grupo "
            "apenas o que é <b>demanda de verdade</b> nas áreas de Direito "
            "<b>Imobiliário</b> e <b>Empresarial</b>: cliente procurando "
            "advogado e colega procurando parceria.",
            "",
            "Tudo passa por uma triagem antes — anúncio de advogado, área fora "
            "do escopo e conversa de grupo ficam de fora.",
        ]

        links = []
        if self.site_url:
            links.append(f'🌐 <a href="{html.escape(self.site_url)}">Site</a>')
    ...
    def texto_suporte(self) -> str:
        if self.suporte_telegram:
            ...
            return (f'💬 Precisa de ajuda? Fale com a gente: '
                    f'<a href="https://t.me/{html.escape(alvo)}">@{html.escape(alvo)}</a>')
        return "💬 Precisa de ajuda? Fale diretamente com o escritório."

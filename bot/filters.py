...
# ---------------------------------------------------------------------------
# Divulgação: advogado se anunciando
# ---------------------------------------------------------------------------
#
# É o ruído dominante em grupo de advogado, e o mais barato de reconhecer: quem
# se anuncia usa um vocabulário de anúncio, sempre o mesmo. Pegar a maior parte
# aqui poupa uma chamada de IA por post.
#
# O risco desta função é matar demanda boa, então ela é construída para errar
# para o lado de deixar passar: o caso duvidoso vai para o classificador, que lê
# o post inteiro. Um falso negativo custa uma chamada de API; um falso positivo
# custa uma oportunidade que ninguém nunca vai saber que existiu.

# Frases que, sozinhas, já são anúncio. Não existe demanda de cliente que diga
# "tabela de honorários" ou "à disposição dos colegas".
_DIVULGACAO_FORTE = re.compile(
    r"tabela\s+de\s+honor[aá]rios|"
    r"(?:fa[cç]o|elaboro|confecciono|redijo)\s+(?:pe[cç]as|peti[cç][oõ]es|contesta)|"
    r"confec[cç][aã]o\s+de\s+pe[cç]as|"
    r"aceito\s+parcerias?|aceitamos\s+parcerias?|"
...
# Sinais mais fracos: sozinhos não provam nada, mas dois juntos já desenham um
# cartão de visita.
_DIVULGACAO_FRACA = (
    re.compile(r"\bsou\s+advogad", re.I),
    re.compile(r"\bsomos\s+(?:um\s+)?escrit[oó]rio", re.I),
    re.compile(r"escrit[oó]rio\s+especializad", re.I),
    re.compile(r"\bespecialista\s+em\s+direito", re.I),
    re.compile(r"\d+\s*anos?\s+de\s+experi[eê]ncia", re.I),
    re.compile(r"\boab[/\s]*[a-z]{2}\s*[\d.]+", re.I),
    re.compile(r"atendimento\s+(?:online|humanizado|personalizado)", re.I),
    re.compile(r"segue\s+(?:meu|nosso)\s+contato", re.I),
)

# Marcas de quem está PROCURANDO, não oferecendo. Se aparecerem, o pré-filtro se
# cala e deixa o classificador julgar — é exatamente aqui que mora a demanda, e
# um post pode ter as duas coisas ("sou advogado e preciso de um colega em...").
_PEDIDO = re.compile(
    r"\bpreciso\s+de\b|\bprecisa\w*\s+de\s+(?:um\s+)?advogad|\bprocur[oa]\w*\b|"
    r"\balgu[eé]m\s+(?:indica|conhece|atua|faz|pega|sabe)\b|"
    r"\bestou\s+precisando\b|\bquem\s+atua\b|\bindica[cç][aã]o\s+de\s+advogad|"
    r"\btenho\s+(?:uma\s+)?demanda\b|\bpasso\s+(?:o\s+)?caso\b|"
    r"\bh[aá]\s+interesse\b|\btem\s+interesse\b",
    re.I,
)


def parece_divulgacao(texto: str) -> bool:
    """O post é um advogado se anunciando?

    Devolve True só quando o sinal é forte **e** não há marca de pedido no
    texto. Na dúvida devolve False e o custo é uma chamada de classificador.
    """
    texto = texto or ""
    if not texto.strip():
        return False

    # Está pedindo alguma coisa: não é o caso de cortar de graça.
    if _PEDIDO.search(texto):
        return False

    if _DIVULGACAO_FORTE.search(texto):
        return True

    return sum(1 for padrao in _DIVULGACAO_FRACA if padrao.search(texto)) >= 2
